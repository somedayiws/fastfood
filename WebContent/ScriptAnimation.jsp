...
<script type="text/javascript">
	$(function() {
		...
		setInterval(function() {
			$('.neoslideshow :first-child').fadeOut(400).next('img').fadeIn(400)
					.end().appendTo('.neoslideshow');
		}, 5000);
	});
	/*Phần này không cần thiết từ đây ...*/
	function LoadPage(bien) {
		alert("Load lại trang : " + bien);
		$('.giohang').load('GioHangOnline.jsp', {
			data : 1
		}, thongbao);
	}

	function LoadPage1(bien) {
		alert("Load lại trang : " + this.id);
	...
	/*Phần này không cần thiết đến đây*/
	$(function() {
		...
		function left_slide_circle() {
			...
			$('.slide-stage').stop().animate({
				left : 0
			}, anispeed); // Thực hiện animation
		...
		function right_slide_circle() {
			...
			$('.slide-stage').css("left",
					-(stage_width - image_width - display_width) + "px"); // Dịch chuyển tức thời vị trí của stage sang một khoảng bằng 1 slide-image
			$('.slide-stage').stop().animate({
				left : display_width - stage_width
			}, anispeed); // Thực hiện animation
			$('.swapthis').removeClass('swapthis'); // Reset class cho slide-image vừa chuyển
		}
		...
		$(".slide-container,.slide-control-prev,.slide-control-next").hover(
				function() {
					clearInterval(play);
				}, function() {
					start_slide_auto_scroll();
